...
namespace MapGen
{
    internal static class BasicGenerationMethods
    {
        ...
        {
            { RoomSize.empty, (0, 0, 40, Data.LEVEL1_ONE_ONE) },
            ...
            for (int i = 0; i < roomDimensions.SizeX; i++)
            {
                for (int j = 0; j < roomDimensions.SizeY; j++)
                {
                    if (i == 0)
                    {
                        ...
                        if (MapEngine.chunkMap[startX + i - 1, startY + j].type == ChunkType.main)
                            CreateExit(chunk, 3);
                    }
                    ...
                    if (j == 0)
                    {
                        ...
                        if (MapEngine.chunkMap[startX + i, startY + j - 1].type == ChunkType.main)
                            CreateExit(chunk, 2);
                    }
            ...
            switch (route)
            {
                case 1:
                    for (int i = 1; i < 10; i++)
                    {
                        ...
                        MapEngine.chunkMap[chunk.x + 1, chunk.y].chunkPixelArray[0, i] = '0';
                        MapEngine.rightChanksUpdate.Remove(chunk);
                        MapEngine.leftChanksUpdate.Remove(MapEngine.chunkMap[chunk.x + 1, chunk.y]);
                    }
                    break;
                case 2:
                    for (int i = 1; i < 10; i++)
                    {
                        ...
                        MapEngine.chunkMap[chunk.x, chunk.y - 1].chunkPixelArray[i, 10] = '0';
                        MapEngine.upChanksUpdate.Remove(chunk);
                        MapEngine.downChanksUpdate.Remove(MapEngine.chunkMap[chunk.x, chunk.y - 1]);
                    }
                    break;
                case 3:
                    for (int i = 1; i < 10; i++)
                    {
                        ...
                        MapEngine.chunkMap[chunk.x - 1, chunk.y].chunkPixelArray[10, i] = '0';
                        MapEngine.leftChanksUpdate.Remove(chunk);
                        MapEngine.rightChanksUpdate.Remove(MapEngine.chunkMap[chunk.x - 1, chunk.y]);
                    }
                    break;
                case 4:
                    for (int i = 1; i < 10; i++)
                    {
                        ...
                        MapEngine.chunkMap[chunk.x, chunk.y + 1].chunkPixelArray[i, 0] = '0';
                        MapEngine.downChanksUpdate.Remove(chunk);
                        MapEngine.upChanksUpdate.Remove(MapEngine.chunkMap[chunk.x, chunk.y + 1]);
                    }
                    break;
            }
        ...
        public static void CheckXYRight(int startX, int startY, out int sizeX, out int sizeYUp, out int sizeYDown)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (MapEngine.chunkMap[startX + i, startY].type != ChunkType.empty)
                {
                    sizeX = i;
                    sizeYUp = maxJUp;
                    sizeYDown = maxJDown;
                ...
                for (int j = 1; j <= 4; j++)
                {
                    if (MapEngine.chunkMap[startX + i, startY - j].type != ChunkType.empty)
                    {
                        if (maxJUp > j)
                            maxJUp = j - 1;
        ...
        public static void CheckXYUp(int startX, int startY, out int sizeY, out int sizeXRight, out int sizeXLeft)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                if (MapEngine.chunkMap[startX, startY - j].type != ChunkType.empty)
                {
                    sizeY = j;
        ...
        public static void BuildRoom(int startX, int startY, RoomSize roomSize)
        {
            ...
            for (int i = 0; i < roomDimensions.SizeX; i++)
            {
                for (int j = 0; j < roomDimensions.SizeY; j++)
                {
                    MapEngine.chunkMap[startX + i, startY + j].type = ChunkType.main;
                    MapEngine.chunkMap[startX + i, startY + j].size = roomSize;
                    ...
                    MapEngine.chunkMap[startX + i, startY + j].chunkPixelArray = chunkPixelArray;
                }
            }
        ...
        public static bool BuildRoomRight(int startX, int startY, bool isMain)
        {
            ...
            for (int i = 0; i < roomDimensions.SizeX; i++)
            {
                for (int j = 0; j < roomDimensions.SizeY; j++)
                {
                    MapEngine.chunkMap[startX + i, randomStartY + j].type = chunkType;
                    MapEngine.chunkMap[startX + i, randomStartY + j].size = roomSize;
                    ...
                    MapEngine.chunkMap[startX + i, randomStartY + j].chunkPixelArray = chunkTilesArray;
                }
            }
        ...
        public static bool BuildRoomLeft(int startX, int startY, bool isMain)
        {
            ...
            for (int i = 0; i < roomDimensions.SizeX; i++)
            {
                for (int j = 0; j < roomDimensions.SizeY; j++)
                {
                    MapEngine.chunkMap[startX + i - roomDimensions.SizeX + 1, randomStartY + j].type = chunkType;
                    MapEngine.chunkMap[startX + i - roomDimensions.SizeX + 1, randomStartY + j].size = roomSize;
                    ...
                    MapEngine.chunkMap[startX + i - roomDimensions.SizeX + 1, randomStartY + j].chunkPixelArray = chunkPixelArray;
                }
            }
        ...
        public static bool BuildRoomUp(int startX, int startY, bool isMain)
        {
            ...
            for (int i = 0; i < roomDimensions.SizeX; i++)
            {
                ...
                {
                    MapEngine.chunkMap[randomStartX + i, startY - j].type = chunkType;
                    MapEngine.chunkMap[randomStartX + i, startY - j].size = roomSize;
                    ...
                    MapEngine.chunkMap[randomStartX + i, startY - j].chunkPixelArray = chunkPixelArray;
                }
            }
